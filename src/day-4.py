...
def day_four_p1(in_file):
    ...
    def count_matches(matrix, pattern):
        ...
        # Horizontal Search
        count += np.sum(np.all(rolling_window(matrix, pattern_length) == pattern, axis=-1))

        # Vertical Search
        count += np.sum(np.all(rolling_window(matrix.T, pattern_length) == pattern, axis=-1))

        # Diagonal Search (Top-Left to Bottom-Right)
        def extract_diagonals(mat, length):
            diagonals = [mat.diagonal(i) for i in range(-mat.shape[0] + length, mat.shape[1] - length + 1)]
            diagonals = [d for d in diagonals if len(d) >= length]
            return diagonals

        diagonals = extract_diagonals(matrix, pattern_length)
        count += sum(np.sum(np.all(rolling_window(diag, pattern_length) == pattern, axis=-1)) for diag in diagonals)

        # Diagonal Search (Top-Right to Bottom-Left)
        flipped_matrix = np.fliplr(matrix)
        diagonals_flipped = extract_diagonals(flipped_matrix, pattern_length)
        count += sum(np.sum(np.all(rolling_window(diag, pattern_length) == pattern, axis=-1)) for diag in diagonals_flipped)
...
def day_four_p2(in_file):
    ...
    for position in positions_of_3:
        ...
        if i > 0 and i < rows - 1 and j > 0 and j < cols - 1:
            ...
            if ((char_matrix[i-1, j-1] == 2 and char_matrix[i+1, j+1] == 4) or  # "M -> A -> S"
                (char_matrix[i-1, j-1] == 4 and char_matrix[i+1, j+1] == 2)):   # "S -> A -> M"
